  ...
  return (
    <div className="p-4 lg:p-6 space-y-6 max-w-6xl mx-auto">
            ...
            return (
              <div key={step.id} className="flex items-center flex-1">
                <div className={`flex items-center gap-3 ${isActive ? "opacity-100" : isCompleted ? "opacity-70" : "opacity-40"}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${isCompleted ? "bg-green-500 text-white" : isActive ? "bg-[#006400] text-white" : "bg-gray-200 text-gray-500"}`}>
                ...
                {idx < WORKFLOW_STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-4 ${isCompleted ? "bg-green-400" : "bg-gray-200"}`} />}
              </div>
            );
          })}
        </div>
      ...
      {invoice.status === "draft" && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center justify-between no-print">
          <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowSubmitConfirm(true)} className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg shadow-sm">
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 no-print">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowEmailPreview(true)} className="flex gap-2 items-center px-4 py-2 bg-white border border-green-300 text-green-700 rounded-lg shadow-sm font-medium hover:bg-green-100">
                <Mail className="w-4 h-4" /> Email Client
              </button>
              <button onClick={() => setShowEmailPreview(true)} className="flex gap-2 items-center px-6 py-2 bg-[#006400] text-white rounded-lg shadow-sm font-bold hover:bg-[#004d00]">
